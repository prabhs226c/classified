(function ($) {
    $(document).ready(function () {
        ...
        jQuery(document).on('click', '#qucikad-ajaxsearch-dropdown ul li', function (event) {
            ...
            jQuery("#catIdHidden").val(dropId);
            getSubCategories(dropId);
            jQuery("#sub-keywords-div").show();
...
function getSubCategories(dropId){
    ...
    request.done(function( msg ) {
        if(msg['error'])
        {
            ...
            $('#subCat-dropdown-List-quckiad').html(msg);

        }
    });
}

jQuery('#sub-keywords').on('click', function (event) {

    jQuery("#qucikad-ajaxsearch-dropdown-subCat").niceScroll({
        cursorcolor: "#c9c9c9",
        cursoropacitymax: 1,
        boxzoom: false,
        cursorwidth: "8px",
    ...
    jQuery("#qucikad-ajaxsearch-dropdown-subCat").attr('size',  jQuery('#qucikad-ajaxsearch-dropdown-subCat > option').length);
    jQuery("#qucikad-ajaxsearch-dropdown-subCat").css('display', 'block');

});

jQuery(document).on('blur', '#sub-keywords', function (event) {
    $('#qucikad-ajaxsearch-dropdown-subCat').attr('size', 0);
    $('#qucikad-ajaxsearch-dropdown-subCat').css('display', 'none');
})
jQuery(document).on('click', '#qucikad-ajaxsearch-dropdown-subCat ul li', function (event) {
    var subCategory = jQuery.trim(jQuery(this).text());
    var subCatId = $(this).attr('data-id');
    jQuery('#dbSubCategory').val(subCatId);
    jQuery('#input-subcat').val(subCatId);
    jQuery('#sub-keywords').val(subCategory);
    $('#qucikad-ajaxsearch-dropdown-subCat').css('display', 'none');
})
