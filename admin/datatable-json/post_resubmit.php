...
foreach ($queryRecords as $row) {
    ...
    $row0 = '<td>
                <label class="css-input css-checkbox css-checkbox-default">
                    <input type="checkbox" class="service-checker" value="'.$product_id.'" id="row_'.$product_id.'" name="row_'.$product_id.'"><span></span>
                </label>
            </td>';
    $row1 = '<td class="text-center">
                <div class="pull-left m-r"><img class="img-avatar img-avatar-48" src="../storage/products/'.$image.'"></div>
                <p class="font-500 m-b-0"><a href="post_detail.php?id='.$product_id.'&resubmit=1" target="_blank">'.$title.'</a>'.$premium.'</p>
